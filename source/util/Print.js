Inspec.util.print = (function(){
  var printArray = function(obj, opts) {
    var result = [];
    for (var i = 0; i < Math.min(opts.maxArray, obj.length); i++)
      result.push(print(obj[i], Inspec.merge(opts, { maxArray: 3, maxString: 40 })));
  ...
  var printObject = function(obj, opts) {
    ...
    for (var k in obj) {
      if (obj.hasOwnProperty(k) && inArray(obj[k], seen) < 0) {
        seen.push(obj[k]);
        value = print(obj[k], Inspec.merge(opts, { maxArray: 6, maxString: 40, seen: seen }));
      } else
        value = "...";
      result.push(k + ": " + value);
    }
  ...
  var print = function(obj, options) {
    var opts = Inspec.merge({ maxArray: 10, maxString: 100 }, options);
    ...
    if (typeof obj == 'undefined')
      ...
      return printObject(obj, Inspec.merge(options, { maxString: 200 }));
